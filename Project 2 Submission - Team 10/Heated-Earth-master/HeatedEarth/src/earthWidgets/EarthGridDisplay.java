...
public class EarthGridDisplay extends JPanel {
	...
	private TemperatureColorPicker colorPicker = TemperatureColorPicker
			.getInstance();

	private BufferedImage imgTransparent;
	private BufferedImage earthImage;
	private float[] scales = { 1f, 1f, 1f, OPACITY }; // last index controls the
														// transparency
	...
	void setGranularity(int degreeSeparation) {
		...
		imgTransparent = new BufferedImage(imgWidth, imgHeight,
	...
	private void fillCellColors(Graphics g) {
		...
		for (int x = 0; x < numCellsX; x++) {
			for (int y = 0; y < numCellsY; y++) {
				...
				int cellHeight = (int) getCellHeight(y);
	...
	private int getCellHeight(int col) {
		int numRows = 180 / degreeSeparation;

		int lat1 = 90 - (col * degreeSeparation);
		int lat2 = 90 - ((col + 1) * degreeSeparation);

		int y1 = (int) Util.getDistToEquator(lat1, radius);
		int y2 = (int) Util.getDistToEquator(lat2, radius);
