...
public abstract class Util {
	...
	public static float getLatitudeCircum(double latitude, double earthRadius) {
		double latRadius = earthRadius
				* Math.sin(Math.toRadians(90d - latitude));
	 ...
	 */
	public static float getTrapezoidArea(double topLength, double bottomLength,
			double height) {
	 ...
	 */
	public static float getTrapezoidSideLen(double topLength,
			double bottomLength, double height) {
		return (float) Math.sqrt(Math.pow(
				(Math.abs(topLength - bottomLength) / 2), 2)
				+ Math.pow(height, 2));
	}
